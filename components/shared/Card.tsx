...
const Card = async ({ event, hasOrderLink, hidePrice }: CardProps) => {
  ...
  const userOrders = await getUserOrdersForEvent(event._id, userId);
  ...
  return (
    ...
    hover:shadow-lg md:min-h-[438px]'>
      {/* NUMBER OF TICKETS RIBBON */}
      {userOrders.length > 1 && (
        <div className="absolute top-2 left-2 bg-primary/80 text-white px-2 py-1 rounded-full z-10">
